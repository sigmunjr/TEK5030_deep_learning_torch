...
def run_live():
    ...
    while ret:
        ...
        output = net(transforms(LiveDataset.convert_to_torch_image(image, device).unsqueeze(0)))
        output = output.argmax(1).cpu().numpy()[0]
        image = cv2.putText(image, 'label: ' + str(output), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 50, 50), 3)
